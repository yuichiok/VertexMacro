...
void chargeEfficiency2()
{
	...
	std::string recofile_before = rootfiles[token].filename();
	std::cout << std::endl;

	cout << "TRP (before VR) = " << recofile_before << endl;

	std::cout << "TRP after: ";
	std::cin >> token;

	std::string recofile_after = rootfiles[token].filename();
	std::cout << std::endl;

	cout << "TRP (after VR) = " << recofile_after << endl;

	std::cout << "TVF: ";
	std::cin >> token;

	std::string genfile = rootfiles[token].filename();
	std::cout << std::endl;

	cout << "TVF = " << genfile << endl;
	...
	//RECO2->SetBranchAddress("numberOfTagged", &_numberOfTagged);
	//RECO2->SetBranchAddress("PDG", _PDGreco);
	//RECO2->SetBranchAddress("charge", _chargereco);
	//RECO2->SetBranchAddress("generation", _generationreco);
	//RECO2->SetBranchAddress("numberOfParticles", _numberOfParticles);
	...
	//RECO2->SetBranchAddress("bteta", &_bteta);
	//RECO2->SetBranchAddress("bbarteta", &_bbarteta);
