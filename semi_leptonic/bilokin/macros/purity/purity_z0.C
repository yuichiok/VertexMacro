...
*/
	...
	filename = "/group/ilc/users/yokugawa/TTbar/l5/eLpR/semiLep/electron/vertex_restorer_out/root_merge/eLpR_electron_VR_Z0corr2.root";
	...
	float maxangle = 0.1;
	float maxdev = 20;
	TH2F * rec = new TH2F("rec", ";#alpha;z_{0}/#sigma_{Z_{0}}", nbin,0,maxangle,nbin,0,maxdev );
	TH2F * fake = new TH2F("fake", ";#alpha;z_{0}/#sigma_{Z_{0}}", nbin,0,maxangle,nbin,0,maxdev );
	TCanvas * c1 = new TCanvas("c1", "Data-MC",0,0,1500,500);
	c1->Divide(3,1);
	c1->cd(1);
	Stats->Project("rec","(std::abs(missedZ0)/sqrt(missedZ0Error)):missedAngle","missedMomentum > 1. && missedObs > 0");
	...
	Stats->Project("fake","(std::abs(fakeZ0)/sqrt(fakeZ0Error)):fakeAngle","fakeMomentum > 1. && fakeObs > 0");
	...
	TH2F * purity = new TH2F("purity", ";#alpha;z_{0}/#sigma_{Z_{0}}", nbin,0,maxangle,nbin,0,maxdev );
	TH2F * efficiency = new TH2F("efficiency", "efficiency map;#alpha;z_{0}/#sigma_{Z_{0}}", nbin,0,maxangle,nbin,0,maxdev );
	int fakesum = 0;
	int recsum = 0;
	...
	//c1->cd(4);
	TH2F * sum = new TH2F("sum", "missed;#alpha;z_{0}/#sigma_{Z_{0}}", nbin,0,maxangle,nbin,0,maxdev );
	sum->Add(rec, fake);
